...
REACTOME <- REACTOME[order(REACTOME$Percent , decreasing = TRUE), ]

# Subset to non-redundant terms, removing terms with 100% gene overlap keeping top term (ranked by p-value)
rm(REACTOME_ordered)
for(i in unique(REACTOME$geneID)){
  tmp <- REACTOME[REACTOME$geneID == i, ]
  
  if(exists('REACTOME_ordered')){
    REACTOME_ordered <- rbind(REACTOME_ordered, tmp)
  } else {
    REACTOME_ordered <- tmp
  }
}

# Plot non-redundant terms
REACTOME_ordered %>%
  mutate(Description = factor(Description, levels = rev(REACTOME_ordered$Description))) %>%
  ggplot() +
