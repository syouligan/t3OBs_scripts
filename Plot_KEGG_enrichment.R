...
# Load KEGG enrichment data
KEGG <- read.csv("project_results/t3OBs/Enriched_KEGG_T3ob_DEG_pairwise_list.csv", header = TRUE)

# Extract number of genes in each group
KEGG <- separate(data = KEGG, col = BgRatio, into = c("Group", "Total"), sep = "\\/")
KEGG$Group <- as.numeric(KEGG$Group)
KEGG$Percent <- KEGG$Count / KEGG$Group *100
...
KEGG %>%
  ...
  scale_fill_gradientn(aes(fill = -log10(p.adjust)), colors = c("orange", "purple"), limits = c(0, max(-log10(KEGG$p.adjust))), values = NULL, space = "Lab", na.value = "grey80", guide = "colourbar", aesthetics = "fill") +
  ...
  labs(x = "% of KEGG genes", x = NA) +
  theme_classic() +
  ggsave("project_results/t3OBs/KEGG_signature_percent_dotplot.pdf", useDingbats = FALSE)
