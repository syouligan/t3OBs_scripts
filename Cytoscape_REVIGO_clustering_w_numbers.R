...
idx <- match(BPsig$Name, cluster_nums$Name)
BPsig$Number <- cluster_nums$Number [idx]
...
ggnet2(net, color = "Top_genes", size = "p.adjust", alpha = 0.5, size.cut = TRUE, color.legend = "Cluster", palette = getPalette, label = 'Numbers', label.size = 2, label.color = "grey20") +
  ggsave("project_results/t3OBs/Revigo_GOBP_clustering_cytoscape_w_numbers.pdf")

write.csv(BPsig, "project_results/t3OBs/Enriched_GOBP_T3ob_DEG_pairwise_list_network_annotated.csv")
