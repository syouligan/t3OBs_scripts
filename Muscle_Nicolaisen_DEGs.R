...
# --------------------------------------------------------------------------
# Find genes differentially expressed in Muscle_Nicolaisen
# --------------------------------------------------------------------------
...
# Set
set = "Muscle_Nicolaisen"

# Load data files and remove trailing decimal from gene ids
gene_count <- read.csv(paste0('data/public_data/', set, '/WT_ALPHA_V1_T3_counts.csv'), row.names = 1, header = TRUE)
...
sampleTable <- data.frame(sampleName, sampleType, sampleRep, sampleTreatment, sampleBackground)
...
for(grp in groups){
  ...
  binarised[, paste0(grp, "_mean")] <- rowMeans(log2(gene_count[, cols] + 1))
  
  idx <- match(rownames(geneActivity), rownames(binarised))
  geneActivity[,paste0(grp, "_muscle_activity")] <- binarised[,paste0(grp, "_activity")] [idx]
  geneActivity[,paste0(grp, "_muscle_mean")] <- binarised[,paste0(grp, "_mean")] [idx]
...
for(cont in colnames(contrasts)) {
  ...
  genemap_tmp <- geneActivity[which(geneActivity[,paste0(group1, "_muscle_activity")] == sum(grepl(group1, sampleTable$sampleType)) | geneActivity[,paste0(group2, "_muscle_activity")] == sum(grepl(group2, sampleTable$sampleType))), ]
