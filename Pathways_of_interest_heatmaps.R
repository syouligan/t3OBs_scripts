...
DEGs <-read.csv("summary/DEGs_LFC_0.15_pairwise.csv", header = TRUE)
...
pathways <- list('RANK_RANKL_OPG' = RANK_RANKL_OPG, 'WNT' = WNT, 'TGFB_BMP_SMAD' = TGFB_BMP_SMAD, 'EPHRIN_EPH' = EPHRIN_EPH, 'ESTROGEN' = ESTROGEN, 'SEMAPHORIN_PLEXIN' = SEMAPHORIN_PLEXIN, "GLYCOLYSIS" = GLYCOLYSIS, "PYRUVATE" = PYRUVATE, "TCA" = TCA, "RESP_TRANSPORT" = RESP_TRANSPORT, "MITO_ATP" = MITO_ATP, "MITO_UNCOUPLING" = MITO_UNCOUPLING, 'GOIs' = GOIs, "DEGs" = DEGs, "DEGs_0LFC" = DEGs_0LFC)
pathways <- list('GLYCOLYSIS_LEE' = GLYCOLYSIS_LEE, 'TCA_LEE' = TCA_LEE, 'FATTYACID_LEE' = FATTYACID_LEE, 'MITOGENESIS_LEE' = MITOGENESIS_LEE, 'ETCC1_LEE' = ETCC1_LEE, 'ETCC2_LEE' = ETCC2_LEE, "ETCC3_LEE" = ETCC3_LEE, "ETCC4_LEE" = ETCC4_LEE, "ETCC5_LEE" = ETCC5_LEE)

# Cap zscores at 1.75, -1.75
CPMzscores_long$Expression[CPMzscores_long$Expression < -1.75] <- -1.75
CPMzscores_long$Expression[CPMzscores_long$Expression > 1.75] <- 1.75

# Plot pathways of interest
for(i in names(pathways)) {
  
  hc <- hclust(dist(CPMzscores[CPMzscores$GeneSymbol %in% pathways[[i]], 1:12]))$order
  hc_order <- CPMzscores$GeneSymbol[CPMzscores$GeneSymbol %in% pathways[[i]]][hc]
  
  CPMzscores_long %>%
    filter(GeneSymbol %in% pathways[[i]]) %>%
    mutate(Sample = factor(Sample, levels = c("WTV1_4", "WTV1_5", "WTV1_6", "WTT3_4", "WTT3_5", "WTT3_6", "ALPHAV1_4", "ALPHAV1_5", "ALPHAV1_6", "ALPHAT3_4", "ALPHAT3_5", "ALPHAT3_6"))) %>%
    ...
    ggsave(paste0("project_results/t3OBs/Pathway_", i, "_expression.pdf"))
  }
