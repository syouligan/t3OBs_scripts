...
for(set in c("BP", "CC", "MF")) {
  ...
  GOsig <- read.csv(paste0("project_results/t3OBs/Enriched_GO", set, "_T3ob_DEG_pairwise_list.csv"), header = TRUE)
  ...
  # Import Revigo.xgmml
  x1 <- read_xml(paste0("project_results/t3OBs/Revigo_", set, ".xgmml"))
  ...
  ggnet2(net, color = "Top_genes", size = "p.adjust", alpha = 0.5, size.cut = TRUE, color.legend = "Cluster", palette = getPalette, label = 'Numbers', label.size = 2, label.color = "grey20") +
    ggsave(paste0("project_results/t3OBs/Revigo_GO", set, "_clustering_cytoscape_w_numbers.pdf"))
  
  write.csv(GOsig, paste0("project_results/t3OBs/Enriched_GO", set, "_T3ob_DEG_pairwise_list_network_annotated.csv"))
