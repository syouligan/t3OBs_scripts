...
# --------------------------------------------------------------------------
# Find genes differentially expressed in Intestine_Godart
# --------------------------------------------------------------------------
...
# Set
set = "Intestine_Godart"

# Load data files and remove trailing decimal from gene ids
gene_count <- read.csv(paste0('data/public_data/', set, '/Total_counts.csv'), header = TRUE)
gene_count <- gene_count[! (duplicated(gene_count$Gene.Symbol) | is.na(gene_count$Gene.Symbol)), ]
rownames(gene_count) <- gene_count$Gene.Symbol
gene_count <- gene_count[,2:7]
...
contrasts <- makeContrasts( V1_T3 = sampleTypeT3-sampleTypeV1,
                            levels = design )
...
for(grp in groups){
  ...
  idx <- match(geneActivity$GeneSymbol, rownames(binarised))
...
for(cont in colnames(contrasts)) {
  ...
  genemap_tmp <- genemap_tmp[!duplicated(genemap_tmp$GeneSymbol),]
  expression_tmp <- gene_count[rownames(gene_count) %in% genemap_tmp$GeneSymbol,]
  genemap_tmp <- genemap_tmp[match(rownames(expression_tmp), genemap_tmp$GeneSymbol),]
  ...
  idx <- match(geneActivity$GeneSymbol, rownames(res))
