...
# --------------------------------------------------------------------------
# Find genes differentially expressed in Cardiomyocytes_Hirose
# --------------------------------------------------------------------------
...
# Set
set = "Cardiomyocytes_Hirose"

# Load data files and remove trailing decimal from gene ids
gene_count <- read.csv(paste0('data/public_data/', set, '/WT_ALPHAKO_counts.csv'), row.names = 1, header = TRUE)
...
contrasts <- makeContrasts( WT_ALPHAKO = sampleTypeALPHAKO-sampleTypeWT,
                            levels = design )
