...
for(cluster in c("cluster_1", "cluster_3", "cluster_4")) {
  # Load significantly enriched GO terms
  GOsig <- read.csv(paste0("project_results/t3OBs/DGE_HC/HC_DEGs_0.15LFC_", cluster, "_Enriched_GOBP.csv"), header = TRUE)
  # GOsig <- GOsig[GOsig$p.adjust < ifelse(set == 'BP', 0.05, 0.1),]
  ...
  # Import Revigo.xgmml
  x1 <- read_xml(paste0("project_results/t3OBs/DGE_HC/Revigo_BP_", cluster, ".xgmml"))
  ...
  ggnet2(net, color = "Top_genes", size = "p.adjust", alpha = 0.5, size.cut = TRUE, color.legend = "Cluster", palette = getPalette, label = 'Numbers', label.size = 2, label.color = "grey20") +
    ggsave(paste0("project_results/t3OBs/DGE_HC/Revigo_GOBP_clustering_cytoscape_w_numbers_", cluster,".pdf"))
  
  write.csv(GOsig, paste0("project_results/t3OBs/DGE_HC/Enriched_GOBP_T3ob_DEG_pairwise_list_network_annotated_", cluster,".csv"))
